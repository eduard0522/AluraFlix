  ...
  return (
    <section className="md:text-center lg:text-left">
       ...
       <div className="flex flex-row mx-auto overflow-auto gap-8 py-8 w-full md:justify-center lg:justify-start ">
           { videos.filter((video) => video.group === group.name).map(video => <CardVideo key={video.id} group={ group } video={video} scrollToSection={scrollToSection} videosDates={videosDates}/>) }
       </div>
    </section>
  )
}
